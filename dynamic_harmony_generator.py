...
def generate_dynamic_harmony(sequence, sample_rate=44100, attack=0.05, decay=0.05, sustain_level=0.7, release=0.1):
    samples = []
    for segment in sequence:
        if segment["type"] == "chord":
            ...
            chord_wave = chords_to_wave([chord], sample_rate, wave_type=segment["wave_type"], use_adsr=True,
                                        attack=attack, decay=decay, sustain_level=sustain_level, release=release)
            ...
            melody_wave = chords_to_wave([[note] for note in melody], sample_rate, wave_type=segment["wave_type"],
                                         use_adsr=True, attack=attack, decay=decay, sustain_level=sustain_level, release=release)
